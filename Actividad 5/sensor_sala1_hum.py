...
def on_connect(client, userdata, flags, rc, properties=None):
    print("Sensor sala1/hum conectado con código %s" % rc)
...
client = paho.Client(client_id="sensor_sala1_hum", userdata=None, protocol=paho.MQTTv311)
...
while True:
    if running:
        hum = round(random.uniform(18.0, 30.0), 2)
        client.publish("lan/sala1/sensor/hum", payload=str(hum), qos=1)
        print("🌡️ Publicado hum:", hum)
    time.sleep(2)
